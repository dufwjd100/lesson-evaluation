 ...
 * 안양점 출석부 기반 / 학부모용 / 수동 실행
 */

// ───── 설정 ─────
const ATTENDANCE_SHEET_ID = '1QOp8T83xyyBHt_BU3RAbytcqeVrEuHB3nED6zeLw_d4';
...
【기존평가 활용】
- 각 기록의 기존평가가 비어 있지 않으면 선생님이 실제로 쓰는 말투, 문장 길이, 부드러운 표현 방식, 약간의 인간적인 여백을 우선 참고하세요.
- 기존평가가 충분히 있으면 딱딱한 레슨 리포트보다 선생님이 직접 남기는 짧은 코멘트처럼 작성하세요.
- 기존평가의 문장을 그대로 복사하지 말고, 이번 최근 4회 수업기록에 맞게 새로 작성하세요.
- 기존평가에 있는 칭찬이나 판단을 현재 수업기록 근거 없이 반복하지 마세요.
- "진행하였습니다", "목표로", "지도하겠습니다"처럼 건조한 보고서 표현은 꼭 필요할 때만 쓰고, 기존평가처럼 자연스러운 말투를 우선하세요.
...
- 총 2문단만 작성합니다.
- 각 문단은 1~2문장으로 작성합니다.
- 전체 220~420자 내외로 작성합니다.
- 첫 문단: 최근 4회 전체에서 반복되거나 이어진 개별 연습들과 각 연습의 목적.
- 둘째 문단: 실제 변화 또는 현재 병목 1개 + 다음 수업 방향 1개.
- 평가 초안 본문만 출력합니다.

【정확도 규칙】
- 마지막 1회 수업만 요약하지 말고, 최근 4회 수업기록 전체를 종합하세요.
- 한 회차에서만 생략되거나 건너뛴 내용을 전체 기간의 방향처럼 쓰지 마세요.
...
- "이번 수업에서는", "최근 수업에서는", "최근 4회의 수업에서", "이번 기간 동안", "수업을 통해"로 시작하지 마세요.
